...
const quizTemplate = (data = [], dataLength = 0, options) => {
  ...
  const answers = data.answers.map((item) => {
    if (item.type == "checkbox") {
      ...
      <img src="img/sneaker-img.jpg" alt="">
        <label class="custom-checkbox quiz-question__label">
          <input class="custom-checkbox__field quiz-question__answer" type="${
            item.type
  ...
  return `
        ...
        <div class="quiz-question__count">${number} из ${dataLength}</div>
				<button type="button" class="btn-reset btn btn--thirdly quiz-question__btn" data-next-btn>${nextBtnText}</button>
        </div>
			</div>
	`;
};
